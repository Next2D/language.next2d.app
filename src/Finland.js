...
class Finland extends Language
{
    ...
    createMapping ()
    {
        return new Map([
            ...
            ["{{中央下}}", "alempi keskitaso (esim. luokassa)"],
            ["{{右下}}", "oikean alakulman"],
            ...
            ["{{閉じる}}", "lähellä (esim. kirja, silmät, kokous jne.)."],
            ["{{保存}}", "varastointi (yleensä neste, esim. sadevesi, tai padon takana)."],
            ["{{リセット}}", "nollaa"],
            ["{{スクリーン}}", "näyttö"],
            ["{{ショートカット設定}}", "Pikavalinta-asetukset"],
            ["{{レイヤーカラー}}", "kerroksellinen väri (väri)"],
            ...
            ["{{変形}}", "muodonmuutos"],
            ["{{カラー効果}}", "värivaikutus"],
            ["{{フィルター}}", "suodatin (erityisesti kamera)"],
            ...
            ["{{ひとつ前面へ}}", "Yksi eteen."],
            ["{{最前面}}", "tärkein"],
            ["{{重ね順}}", "(järjestys) superpositio"],
            ["{{プラグイン}}", "plug-in (ohjelmisto)"],
            ["{{ライブラリ}}", "kirjasto"],
            ["{{プロパティ}}", "omaisuus"],
            ...
            ["{{言語}}", "Kieli."],
            ["{{フォーマット}}", "(tietokone)muoto"],
            ["{{含める}}", "sisältää (vivahde)"],
            ["{{含めない}}", "ei sisällä"],
