...
class Romania extends Language
{
    ...
    createMapping ()
    {
        return new Map([
            ...
            ["{{画質}}", "calitatea imaginii (film, video etc.)"],
            ["{{座標}}", "coordonate"],
            ...
            ["{{中央右}}", "centru-dreapta"],
            ["{{左下}}", "stânga jos"],
            ["{{中央下}}", "mediu-jos (de exemplu, în clasă)"],
            ["{{右下}}", "dreapta-jos"],
            ...
            ["{{閉じる}}", "aproape (de exemplu, carte, ochi, întâlnire etc.)"],
            ["{{保存}}", "depozitare (de obicei un lichid, de exemplu, apă de ploaie, sau în spatele unui baraj)"],
            ["{{リセット}}", "resetare"],
            ["{{スクリーン}}", "ecran"],
            ["{{ショートカット設定}}", "Setări de comenzi rapide"],
            ["{{レイヤーカラー}}", "culoare stratificată (culoare)"],
            ...
            ["{{単一行}}", "linie simplă"],
            ["{{複数行}}", "multiline (comandă, de exemplu)"],
            ...
            ["{{フィルター}}", "filtru (în special pentru camera foto)"],
            ...
            ["{{言語}}", "Limba."],
            ["{{フォーマット}}", "Format (computer)"],
            ["{{含める}}", "include (o nuanță)"],
            ["{{含めない}}", "să nu includă"],
            ["{{使い方、リファレンス}}", "Utilizare, referință."],
            ["{{非表示レイヤー}}", "strat ascuns"],
            ["{{設定}}", "setare (a unui calculator, a unui fișier etc.)"],
