...
class Indonesia extends Language
{
    ...
    createMapping ()
    {
        return new Map([
            ...
            ["{{画質}}", "kualitas gambar (film, video, dll.)"],
            ...
            ["{{中央右}}", "kanan tengah"],
            ["{{左下}}", "kiri bawah"],
            ["{{中央下}}", "menengah ke bawah (misalnya di kelas)"],
            ["{{右下}}", "kanan-bawah"],
            ...
            ["{{閉じる}}", "dekat (misalnya buku, mata, rapat, dll.)"],
            ["{{保存}}", "penyimpanan (biasanya berupa cairan, misalnya air hujan, atau di belakang bendungan)"],
            ["{{リセット}}", "mengatur ulang"],
            ["{{スクリーン}}", "layar"],
            ["{{ショートカット設定}}", "Pengaturan pintasan"],
            ["{{レイヤーカラー}}", "warna berlapis (warna)"],
            ...
            ["{{書込を許可}}", "izin posting"],
            ["{{単一行}}", "garis sederhana"],
            ["{{複数行}}", "multiline (perintah, misalnya)"],
            ...
            ["{{言語}}", "Bahasa."],
            ["{{フォーマット}}", "format (komputer)"],
            ["{{含める}}", "termasuk (nuansa)"],
            ["{{含めない}}", "tidak termasuk"],
            ["{{使い方、リファレンス}}", "Penggunaan, referensi."],
            ["{{非表示レイヤー}}", "lapisan tersembunyi"],
            ["{{設定}}", "pengaturan (komputer atau file, dll.)"],
