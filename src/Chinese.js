...
class Chinese extends Language
{
    ...
    createMapping ()
    {
        return new Map([
            ...
            ["{{親の階層へ移動}}", "移动到父级层次结构"],
            ["{{ロックされたレイヤーです}}", "锁定的图层"],
            ["{{トゥイーンには複数のオブジェクトを設置できません}}", "多个对象不能放在一个Tween动画上"],
            ["{{名前が重複しています}}", "重复的名字"],
            ["{{名前は必須です}}", "名字是必须的"],
            ["{{基準点}}", "参考点"],
            ["{{自由変形ツール}}", "自由变形工具"],
            ...
            ["{{完了}}", "完成"],
            ["{{ステージ左揃え}}", "对齐舞台左侧"],
            ["{{ステージ中央揃え(水平方向)}}", "对齐舞台中心（水平）"],
            ["{{ステージ右揃え}}", "对齐舞台右侧"],
            ["{{ステージ上揃え}}", "对齐舞台上方"],
            ["{{ステージ中央揃え(垂直方向)}}", "对齐舞台中心（垂直）"],
            ["{{ステージ下揃え}}", "对齐舞台下方"],
            ["{{定規}}", "尺子"],
            ["{{表示}}", "显示"],
            ["{{ショートカット}}", "快捷方式"],
            ...
            ["{{追加ファイル}}", "附加文件"],
            ["{{既存のコンテンツ}}", "现有的内容"],
            ["{{上書きするか、ファイル名を変更して追加するか、キャンセルを選択する事ができます}}", "您可以选择覆盖、重命名、添加文件或取消"],
            ["{{全て上書き}}", "全部覆盖"],
            ["{{全てキャンセル}}", "全部取消"],
            ["{{追加・上書き}}", "添加/覆盖"],
            ["{{キャンセル}}", "取消"],
            ["{{表示確認したいフレームを指定}}", "指定你要检查显示的帧"],
            ["{{ファイル名を指定}}", "指定文件名"],
            ["{{ファイル名}}", "文件名"],
            ["{{終了フレームを指定}}", "指定结束帧"],
            ["{{開始フレームを指定}}", "指定起始帧"],
            ...
            ["{{新規フォルダー}}", "新建文件夹"],
            ["{{新規MovieClip}}", "新建电影剪辑"],
            ["{{枠に合わせてテキストをリサイズ}}", "调整文本大小以适应框架"],
            ["{{テキストに合わせて枠をリサイズ}}", "调整框架大小以适应文本"],
            ["{{リサイズしない}}", "不调整大小"],
            ["{{フォント}}", "字体"],
            ["{{テキスト}}", "文本"],
            ["{{ループ}}", "循环"],
            ...
            ["{{マスクレイヤー}}", "蒙版图层"],
            ["{{通常レイヤー}}", "普通图层"],
            ["{{モーショントゥイーンの削除}}", "删除Tween动画"],
            ["{{モーショントゥイーンの追加}}", "添加Tween动画"],
            ["{{モーダル表示}}", "模式指示"],
            ["{{言語}}", "语言"],
            ["{{フォーマット}}", "格式"],
            ["{{含める}}", "包括"],
            ...
            ["{{ループで再生}}", "循环播放"],
            ["{{1回再生}}", "1次播放"],
            ["{{指定したフレームで固定}}", "固定在指定的帧内"],
            ["{{1回逆再生}}", "1 反向播放"],
            ["{{逆ループで再生}}", "反向循环播放"],
            ["{{画面の拡大・縮小}}", "画面的放大和缩小"],
            ["{{カスタムイージングデータを保存}}", "保存自定义渐变数据"],
            ["{{カスタムイージングデータの読込}}", "读取自定义渐变数据"],
            ["{{プレビューのON/OFF}}", "预览开/关"],
            ["{{フォント枠サイズ}}", "字体帧大小"],
            ["{{フォント枠カラー}}", "字体帧颜色"],
            ["{{オニオンスキン}}", "洋葱皮"],
