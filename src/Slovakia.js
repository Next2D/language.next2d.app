...
class Slovakia extends Language
{
    ...
    createMapping ()
    {
        return new Map([
            ...
            ["{{画質}}", "kvalita obrazu (film, video atď.)"],
            ["{{座標}}", "súradnice"],
            ...
            ["{{中央右}}", "Pravý stred"],
            ["{{左下}}", "vľavo dole"],
            ["{{中央下}}", "nižšia stredná (napr. v triede)"],
            ["{{右下}}", "pravá dolná časť"],
            ...
            ["{{閉じる}}", "blízko (napr. kniha, oči, stretnutie atď.)"],
            ["{{保存}}", "skladovanie (zvyčajne kvapaliny, napr. dažďovej vody, alebo za priehradou)"],
            ["{{リセット}}", "resetovanie"],
            ["{{スクリーン}}", "obrazovka"],
            ["{{ショートカット設定}}", "Nastavenia skratiek"],
            ["{{レイヤーカラー}}", "vrstvená farba (farba)"],
            ...
            ["{{言語}}", "Jazyk."],
            ["{{フォーマット}}", "(počítačový) formát"],
            ["{{含める}}", "zahŕňať (odtienok)"],
            ["{{含めない}}", "nezahŕňať"],
