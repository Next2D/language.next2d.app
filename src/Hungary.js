...
class Hungary extends Language
{
    ...
    createMapping ()
    {
        return new Map([
            ...
            ["{{同名のアイテムが存在します}}", "Léteznek azonos nevű tétel(ek)."],
            ["{{追加ファイル}}", "további fájl"],
            ...
            ["{{閉じる}}", "közel (pl. könyv, szemek, találkozó stb.)"],
            ["{{保存}}", "tárolás (általában folyadék, pl. esővíz, vagy gát mögött)"],
            ["{{リセット}}", "reset"],
            ["{{スクリーン}}", "képernyő"],
            ["{{ショートカット設定}}", "Shortcut beállítások"],
            ["{{レイヤーカラー}}", "réteges szín (szín)"],
            ...
            ["{{言語}}", "Nyelv."],
            ["{{フォーマット}}", "(számítógépes) formátum"],
            ["{{含める}}", "include (egy árnyalat)"],
